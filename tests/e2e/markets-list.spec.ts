...
// These exercise live Polymarket data (market cards, charts, orderbook). Run serially
// and wait for network idle so they don't time out competing for the dev server.
test.describe.serial('markets list (Polymarket-live)', () => {
  ...
  test('category chip filters markets', async ({ page }) => {
    ...
    // URL should update with category param (commit is enough; full load may lag on cold compile)
    await page.waitForURL(/category=politics/, { waitUntil: 'commit' });
